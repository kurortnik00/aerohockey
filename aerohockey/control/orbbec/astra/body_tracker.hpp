...
class BodyTracker : public astra::FrameListener
{
public:
	BodyTracker(World & world) : world (world) { }
		...
		const float jointScale)
	{
		...
		for (const auto& joint : joints)
		{
			...
			if ((type != astra::JointType::LeftHand) && (type != astra::JointType::RightHand) && (type != astra::JointType::LeftFoot) && (type != astra::JointType::RightFoot))
			{
				continue;
			}
			else 
			{
				...
				{
					world.left.moveTo(sf::Vector2f(pos.x, pos.y));
				}
				if (type == astra::JointType::RightHand)
				{
					world.right.moveTo(sf::Vector2f(pos.x, pos.y));
				}
			}

			const auto& pos = joint.depth_position();

			auto color = sf::Color(0xFF, 0x00, 0x00, 0xFF);
			if (joint.status() == astra::JointStatus::NotTracked)
			{
				continue;
			}
			else if (joint.status() == astra::JointStatus::LowConfidence)
			{
				color = sf::Color(0x77, 0x00, 0x00, 0xFF);
			}

			auto radius = jointRadius_ * jointScale * 3.f; // pixels
